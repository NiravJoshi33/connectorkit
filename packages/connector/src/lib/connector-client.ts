...
export class ConnectorClient {
    ...
    private async attemptInstantAutoConnect(): Promise<boolean> {
        ...
        try {
            ...
            setTimeout(() => {
                ...
                if (ws.length > 1) {
                    // Only update if we have more wallets than just our connected one
                    ...
                    console.log(
                        '🎨 Updated wallet list after instant connection, now have',
        ...
        try {
            ...
            if (this.config.debug) {
                ...
                console.log(
                    ...
                    this.state.wallets.map(w => w.wallet.name),
                );
            }
            if (!last) return;
    ...
    async select(walletName: string): Promise<void> {
        ...
        try {
            ...
            this.updateState(
                ...
                true,
            ); // Critical state change - notify immediately
            ...
            this.updateState(
                ...
                true,
            ); // Critical error state - notify immediately
            throw e;
        }
    }
    ...
    async setCluster(clusterId: SolanaClusterId): Promise<void> {
        ...
        if (!cluster) {
            throw new Error(
                `Cluster ${clusterId} not found. Available clusters: ${this.state.clusters.map(c => c.id).join(', ')}`,
            );
        }
    ...
    getDebugMetrics(): ConnectorDebugMetrics {
        ...
        const optimizationRate =
            totalUpdates > 0 ? Math.round((this.debugMetrics.noopUpdates / totalUpdates) * 100) : 0;

        const avgUpdateTime =
            this.debugMetrics.updateTimes.length > 0
