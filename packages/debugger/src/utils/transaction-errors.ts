/**
 * Transaction error parsing utilities
...
export function getSimpleErrorMessage(error?: TransactionErrorType): string {
    ...
    const programError = getTransactionInstructionError(error);
    if (programError) {
        return `Instruction #${programError.index + 1}: ${programError.message}`;
    }
